...
describe('check', () => {
	it('passes a head that contains the baseline and writes the status once', async () => {
		...
		const first = await client.check({ sha: sha(10), report: true });
		...
		const second = await client.check({ sha: sha(10), report: true });
	...
	it('fails a stale head and names the missing tags', async () => {
		...
		const result = await client.check({ sha: sha(10), report: true });
	...
	it('resolves a PR head and skips PRs against other branches by default', async () => {
		...
		const inScope = await client.check({ pr: 7 });
		expect(inScope.sha).toBe(sha(10));
		expect(inScope.written).toBe(true);
		const outOfScope = await client.check({ pr: 8 });
	...
	it('writes a not-applicable pass for other bases when configured', async () => {
		...
		const result = await client.check({ pr: 8 });
	...
	it('rejects an unknown PR as a configuration error', async () => {
		const { client } = harness();
		await expect(client.check({ pr: 99 })).rejects.toThrow(/not found/);
	});
});
...
describe('check guards', () => {
	...
	it('resolves the creator before evaluating anything when reporting', async () => {
		...
		await expect(client.check({ sha: sha(10), report: true })).rejects.toThrow(/--creator/);
	...
	it('fails on a creator mismatch reported by the write', async () => {
		...
		await expect(client.check({ sha: sha(10), report: true })).rejects.toThrow(/expected\[bot\]/);
		expect(github.requests(/\/statuses\//, 'POST')).toHaveLength(1);
	});
	...
	it('rejects a commit together with --pr', async () => {
		const { client } = harness();
		await expect(client.check({ sha: sha(5), pr: 1 })).rejects.toThrow(/either a commit or --pr/);
	});
});

describe('check tag objects', () => {
	it('rejects a tag that resolves to a tree', async () => {
		...
		await expect(client.check({ sha: sha(10), report: true })).rejects.toThrow(/points at a tree/);
	});
});

describe('check round 3', () => {
	it('does not write for a full SHA unless asked', async () => {
		const { client, github } = harness({}, (gh) => {
			gh.tag('pr-baseline', sha(3));
			gh.commit(sha(10), [sha(4)]);
		});
		const result = await client.check({ sha: sha(10) });
		expect(result.verdict.kind).toBe('pass');
...
describe('check validation order', () => {
	...
	it('rejects a cycle of tag objects', async () => {
		...
		await expect(client.check({ sha: sha(10) })).rejects.toThrow(/cycle/);
	});
});

describe('check round 7', () => {
