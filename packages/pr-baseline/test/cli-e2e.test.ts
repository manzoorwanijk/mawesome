...
describe('cli end to end', () => {
	it('exits 0 on a passing check and prints the result as JSON', async () => {
	...
	it('exits 1 on a failing check', async () => {
	...
	it('exits 1 on an incomplete sweep and 0 once it converges', async () => {
		const capped = await run(twoStalePulls, ['sweep', '--max-writes-per-run', '1']);
		expect(capped.status).toBe(1);
		expect(capped.json()).toMatchObject({ written: 1, incomplete: true, reason: 'write-cap' });
		const full = await run(twoStalePulls, ['sweep']);
	...
	it('runs a dry-run move and sweep without writing', async () => {
		const { status, json, github } = await run(
			(gh) => {
				gh.commit(sha(10), [sha(3)]);
				gh.pull({ number: 1, headSha: sha(10) });
			},
			['move-baseline', '--force', '--sweep', '--dry-run'],
		);
		expect(status).toBe(0);
		expect(json()).toMatchObject({ moves: [{ moved: true, to: sha(5) }], sweep: { written: 1 } });
