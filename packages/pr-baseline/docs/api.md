...
`PrepareInput` carries the commits the run will ask about, the open PR numbers and every baseline name with the SHA the API reported (null when absent); `PrepareResult.heads` maps each PR to the head the adapter fetched, null when its ref is gone. Every command calls it before asking about the commits it names (`refresh-pr-status` and `move-baseline` with an empty `pulls` list, `refresh-pr-status` offline with an empty `refs` list, `move-baseline` once more with the labeled merge candidates it found). `refresh-pr-statuses` calls it twice, first for the base head with no PRs, because the scope cannot be settled until the baselines have been checked against that head, then for the selected PR heads alone; the built-in git adapter uses it to fetch in batches and to refuse when a baseline ref on the remote disagrees with the API.

A custom reporter can post a comment, create a check run or forward to a hosted service. `refresh-pr-status` and `refresh-pr-statuses` compare `current()` against the intended status by state, description, target URL and creator, so a reporter that stores no creator should return the configured one. `compareStatus` classes a difference in state or creator as material and one in description or target URL alone as cosmetic; `refresh-pr-status` writes either, and `refreshPrStatuses` spends a write on a cosmetic one only under `scope: 'all'`. A custom reporter always refreshes every PR, since the PR listing's commit statuses are not the reporter's; passing a `scope` other than `'all'` with one is a `ConfigError`. The built-in status reporter is the one exception: the refresh reads its current statuses from the PR listing (one GraphQL page per 100 PRs, two with the git adapter) instead of calling `current()` per PR.
