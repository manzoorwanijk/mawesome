...
This tool grew out of a recurring, hard-to-catch problem in [Gutenberg](https://github.com/WordPress/gutenberg) — a monorepo of 100+ published `@wordpress/*` packages that power the WordPress block editor. Package after package referenced a module in its **published** files — most often `import('react').ReactNode` in an emitted `build-types/*.d.ts` — without declaring `@types/react` (or `csstype`, or another dependency). It worked inside the repo only because the dependency happened to be **hoisted to the root** `node_modules` and resolved ambiently. A consumer installing the package from npm got no such luck: with `skipLibCheck: true` (a common setting) TypeScript silently degraded the affected exports to `any`, and under an isolated install layout (pnpm-style / npm `install-strategy=linked`) the types failed to resolve outright.
...
ESLint and `eslint-plugin-import` operate on **source**, so they never see the **emitted** artifact — and the `dependencies`-vs-`devDependencies` call hinges on whether a type leaks into the **published** `.d.ts`, exactly what a source-level linter can't tell you. The reply in that thread — _"I am exploring the options to fix this via some tool"_ — is what became `dependency-audit`. It analyzes the **released** surface (both `.d.ts` and runtime JS), resolves every reachable import against the package's **declared** ranges materialized fresh (never the hoisted root tree), and flags anything undeclared or unresolvable — so this class of bug fails CI before it ships, instead of being discovered one consumer at a time.
