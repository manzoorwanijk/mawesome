...
- **`src/tools.ts` is the tool registry.** Adding an entry (with a `packages/<slug>/docs/` directory) surfaces the tool in the homepage cards, the sidebar, and — if `playground: true` — a `/<slug>/playground/` route. A docs-only tool just sets `playground: false`. Its `actions` list drives the homepage's GitHub Actions cards; the site depends on each action's workspace only to read its released version.
- **The sidebar shows one tool at a time.** `src/starlightRouteData.ts` narrows Starlight's route data to the current tool's entries (and keeps pagination inside that tool); the `src/components/Sidebar.astro` override heads the list with the tool's name and a link back to the homepage.
